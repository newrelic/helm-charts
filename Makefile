	...
	for chart_name in $(CHARTS); do \
		...
		for example in $${EXAMPLES}; do \
			...
			for value in $${VALUES}; do \
				helm template "$${chart_name}" charts/$${chart_name} --namespace newrelic --set licenseKey='<NR_licenseKey>' --set cluster='<cluser_name>' --set fullnameOverride="$${chart_name}" --values $${value} --output-dir "$${EXAMPLES_DIR}/$${example}/rendered"; \
