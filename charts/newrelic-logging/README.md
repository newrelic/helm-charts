...
> By default, tailing is set to `/var/log/containers/*.log`. To change this setting, provide your preferred path by adding `--set newrelic-logging.fluentBit.path=DESIRED_PATH` to any of the helm install commands above. If you're using a comma-separated list of paths, be sure to escape the commas with a backslash, ie `...path="/var/log/containers/*newrelic*.log\,/var/log/containers/*hello*.log"`
