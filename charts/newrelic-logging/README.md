...
| `fluentBit.sendMetrics`                                      | Enable the collection of Fluent Bit internal metrics in Prometheus format as well as newrelic-fluent-bit-output internal plugin metrics. See [this section](#fluent-bit-metrics-collection) for more details.                                                                                                                                                               | `false`                                                                          |
...
## Fluent Bit metrics collection

The `newrelic-logging` Helm chart uses [Fluent Bit](https://fluentbit.io/) together with New Relic's `newrelic` [OUTPUT plugin](https://github.com/newrelic/newrelic-fluent-bit-output) to send logs to New Relic. Sometimes, despite correctly installing the Helm chart, you may encounter performance issues that affect the correct delivery of logs. In such circumstances, it can be helpful to look at metrics captured by the Fluent Bit processes to figure out what is causing these issues.

By setting the `fluentBit.sendMetrics` configuration option to `true`, you will be able to capture the following metrics:
- **Fluent Bit internal metrics**. These are emitted by Fluent Bit itself in Prometheus format and delivered to New Relic's Prometheus Export endpoint. Because they are faceted by `cluster_name`, `node_name` and `hostname` (pod name), **these are the recommended ones to troubleshoot a Kubernetes cluster environment**. Please refer to this documentation section for instructions on how to easily create a dashboard to represent them.
- **[newrelic-fluent-bit-output](https://github.com/newrelic/newrelic-fluent-bit-output)'s internal plugin metrics**. These metrics are collected by the OUTPUT plugin itself, and are very useful to get insights on the payload packaging and delivery to the New Relic Logs API. They are captured by each OUTPUT plugin instance running in each Fluent Bit process running in the cluster but can only be observed as a whole: they cannot be currently faceted by cluster, node or host. However, it can still be useful to capture them for an entire cluster to be able to observe what is the overall latency when delivering the logs to the New Relic Logs API, observing potential packaging problems or similar. Please refer to [this readme section](https://github.com/newrelic/newrelic-fluent-bit-output?tab=readme-ov-file#troubleshooting-metrics) for instructions on how to easily create a dashboard to represent them.