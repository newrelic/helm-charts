...
| `fluentBit.fluentBitMetrics`                                          | Controls sending Fluent Bit metrics to New Relic. Allowed values: `none`, `essential`, `advanced`. `none`: No metrics sent. `essential`: Only build info metric sent. `advanced`: All metrics sent at 1 minute intervals. Default is `essential`. | `essential`                                                                     |
