...
| gkeAutopilotAllowlist | bool | `false` | Only applies when `provider: GKE_AUTOPILOT`. When true, keeps the `host-fs` mount and the hostmetrics `root_path` that Autopilot normally strips, so `system.filesystem.*` metrics can be collected. Requires a GKE Autopilot allowlist admitting the `/` hostPath (see the GKE Autopilot section of the README). Can be configured also with `global.gkeAutopilotAllowlist`. |
...
### Failed to get instance metadata (resource_detection)
