...
|-----|------|---------|-------------|
| common | object | See `values.yaml` | Common holds config that applies to all instances of the integration. |
...
| common.config.interval | string | `"15s"` | How often the integration should collect and report data. |
...
| controlPlane.enabled | bool | `true` | Enable control plane monitoring. Requires `privileged` mode to be enabled. |
| controlPlane.kind | string | `"DaemonSet"` | How to deploy the control plane scraper. If autodiscovery is in use, it should be `DaemonSet`. Advanced users using static endpoints should be `Deployment` to avoid reporting metrics twice. |
